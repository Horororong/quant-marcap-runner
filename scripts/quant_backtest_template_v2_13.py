...
표준 퀀트 백테스트 템플릿 v2-13 / CURRENT (2026-09 업데이트)
...
6) 채팅 출력은 "실제로 보이는 인터랙티브 차트"를 최우선한다.
   - 버전 2(우선): 네이티브 기간 선택 UI가 지원되면 3개 기본 차트 각각에 기간 선택 버튼을 제공한다.
   - 버전 1(fallback): 버전 2를 안정적으로 렌더링할 수 없으면 3기간 x 3차트 = 9개 인라인 차트를 모두 표시한다.
...
def combine_period_payloads(*payloads: Dict[str, Any]) -> Dict[str, Any]:
    ...
    채팅 UI 계약
    - 버전 2(우선): 기본 그래프 3개(누적자산 / Log2 / Drawdown)를 렌더링하고, 각 그래프 상단에 기간 버튼을 둔다.
    - 기간 버튼: 2001~현재 / 2021~현재 / 최장~현재
    - 기본 선택: 2001~현재
    - 버전 1(fallback): 기간 선택 UI를 안정적으로 구현할 수 없으면 3기간 x 3차트 = 총 9개 인라인 차트를 모두 렌더링한다.
    ...
    return {
        ...
        "presentation_versions": {
            ...
            "version_1": {
                ...
                "use_when": "version_2_unavailable_or_unstable",
        ...
        "charts": chart_defs,
        "chat_render_policy": {
            "preferred_version": "version_2",
            "fallback_version": "version_1",
            "version_2": {
            ...
            "version_1": {
                ...
                "trigger": "native_period_selector_unavailable_or_unstable",
            },
...
9. 차트 출력 방식은 다음 2개 버전으로 정의한다.
   - 버전 2(우선): 위 3개 차트 각각에 동일한 기간 선택 UI를 제공한다.
     · 2001~현재 / 2021~현재 / 최장~현재
     · 기본 선택은 2001~현재
     · 총 인라인 차트 수는 3개다.
   - 버전 1(fallback): 버전 2의 네이티브 기간 선택 UI를 사용할 수 없거나 안정적 렌더링이 불가능하면
     2001~현재 / 2021~현재 / 최장~현재의 3기간 각각에 누적자산 / Log2 / Drawdown을 모두 표시한다.
     · 총 인라인 차트 수는 9개다.
     · 순서: 2001 3개 -> 2021 3개 -> 최장 3개.
10. 버전 2가 가능하면 반드시 버전 2를 우선한다. 단, 실제로 기간 선택 UI가 화면에 렌더링되지 않는다면 버전 2를 제공했다고 간주하지 않는다.
11. 버전 2가 불가능하면 사용자에게 추가 확인을 요구하지 않고 자동으로 버전 1로 fallback한다.
12. 각 그래프에서 전략과 벤치마크 시리즈는 위젯이 지원하는 경우 체크박스/범례 토글로 켜고 끌 수 있게 한다.
13. 책 검증기간은 성과표와 책 수치 비교에는 포함하지만 버전 1/2의 기본 차트에서는 제외한다.
14. 기본 출력 형태는 '성과표 + 버전 2', 버전 2가 불가능하면 '성과표 + 버전 1'이다.
...
21. 새 대화에서도 사용자가 단순히 '백테스트해줘'라고 요청하면 별도 지시 없이 이 CURRENT 템플릿의 계산 및 출력 규칙을 적용한다.
"""
...
def _self_test() -> None:
    ...
    assert combined["chat_render_policy"]["preferred_version"] == "version_2"
    assert combined["chat_render_policy"]["fallback_version"] == "version_1"
