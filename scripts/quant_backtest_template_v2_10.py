"""
quant_backtest_template_v2-10.py

표준 퀀트 백테스트 템플릿 v2-10 (2026-09 업데이트)
...
def combine_period_payloads(*payloads: Dict[str, Any]) -> Dict[str, Any]:
    ...
    fallback_order = [default_period] + [k for k in selector_order if k != default_period]
    return {
        "schema_version": 5,
        "render_target": "chatgpt_chart",
        ...
        "period_selector": {
            "enabled": True,
            "render_mode": "native_if_supported",
        ...
        "chat_render_policy": {
            "priority": "visible_inline_chart_first",
            "capability_check_required": True,
            "native_period_selector": "use_if_supported",
            "fallback_mode": "single_inline_chart",
            "fallback_default_period": default_period,
            "period_fallback_order": fallback_order,
            "chart_order": ["cumulative_wealth", "log2_wealth", "drawdown"],
            "max_inline_widgets_per_message": 1,
...
8. 채팅 인터랙티브 차트는 "실제로 화면에 보이는 것"을 최우선한다.
   - 기간 데이터는 항상 최장~현재 / 2001~현재 / 2021~현재를 모두 생성한다.
   - 현재 채팅 렌더러가 네이티브 기간 선택 UI를 지원하면 기간 선택형으로 렌더링한다.
   - 선택한 기간에서는 누적자산 / 로그2 누적자산 / Drawdown 3종 데이터를 유지한다.
   - 네이티브 기간 선택 UI가 지원되지 않거나 실제 렌더링이 불안정하면 단일 인라인 차트 fallback을 사용한다.
   - fallback 기본기간은 2001~현재이며, 차트 순서는 누적자산 → 로그2 누적자산 → Drawdown이다.
   - 렌더링 안정성이 확인되지 않은 상태에서 한 메시지에 여러 차트를 강제로 삽입하지 않는다.
   - 차트가 화면에 실제로 나타나지 않았으면 "인터랙티브 차트를 제공했다"고 간주하거나 표현하지 않는다.
   - HTML/파일 다운로드형 대시보드는 보조수단이며 채팅 인라인 차트를 대체하지 않는다.
   - 책 검증기간은 성과표에는 포함하되 기본 기간선택 대상에서는 제외한다.
9. 로그2 차트는 log2(NAV)를 직접 그려 1배, 2배, 4배, 8배...가 정확히 동일 간격이어야 한다.
10. 그래프 렌더링 전에 기간, 관측치, 누락치, 열 정렬을 검증한다.
11. 채팅 렌더러의 실제 기능과 템플릿의 희망 UI를 구분한다. 지원되지 않는 UI를 지원되는 것처럼 가정하지 않는다.
"""
