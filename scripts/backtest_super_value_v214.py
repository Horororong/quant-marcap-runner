...
OUT = ROOT / "results" / "super_value_v214"
...
def simulate(panel: pd.DataFrame, selections: dict[pd.Timestamp,pd.DataFrame],
             top_n: int, cost: CostScenario, delist_mode: str = "last_close") -> tuple[pd.DataFrame,pd.DataFrame]:
    ...
    end_date = max(selections)  # stop on last validated signal date before an incomplete next rebalance
...
def metrics(nav: pd.Series, baseline_date: pd.Timestamp, name: str) -> dict:
    s = nav.astype(float).dropna()
    monthly = s.resample("ME").last()
    prior = monthly.index[0] - pd.offsets.MonthEnd(1)
    r = pd.concat([pd.Series([1.0], index=[prior]), monthly]).pct_change().dropna()
    elapsed = (s.index[-1] - pd.Timestamp(baseline_date)).days
    years = elapsed / 365.2425
    final = float(s.iloc[-1])
    cagr = final ** (1.0/years) - 1.0 if years > 0 and final > 0 else np.nan
    stats_r = r
    if pd.Timestamp(baseline_date).to_period("M") == monthly.index[0].to_period("M") and len(r):
        stats_r = r.iloc[1:]
    std = float(stats_r.std(ddof=1)*math.sqrt(12)) if len(stats_r)>=2 else np.nan
    sh = float(stats_r.mean()/stats_r.std(ddof=1)*math.sqrt(12)) if len(stats_r)>=2 and stats_r.std(ddof=1)>0 else np.nan
    dd = drawdown(s)
    rec_days, rec_months = max_recovery(s, baseline_date)
    return {
        "series":name, "start":s.index[0].date().isoformat(), "end":s.index[-1].date().isoformat(),
        "final_multiple":final, "final_asset_krw":final*INITIAL_CAPITAL,
        "cumulative_return":final-1.0, "CAGR":cagr,
        "annualized_volatility_monthly":std, "Sharpe_rf0_monthly":sh,
        "MDD_daily":float(dd.min()), "MDD_date":dd.idxmin().date().isoformat(),
        "max_recovery_days":rec_days, "max_recovery_months":rec_months,
        "monthly_return_count":len(r), "daily_obs":len(s),
    }
...
def main():
    ...
    for cname,cost in COSTS.items():
        nav,turn = simulate(panel,selections_by_n[BASE_TOP_N],BASE_TOP_N,cost,"last_close")
    ...
    robustness_nav = {}
    for n in TOP_NS:
        nav,turn = simulate(panel,selections_by_n[n],n,COSTS["base"],"last_close")
        robustness_nav[f"Top{n}_Base"] = nav["Net"]
        turn["variant"] = f"Top{n}_base"
        turnover_all.append(turn)

    # Delisting worst-case stress for top20.
    stress_nav,stress_turn = simulate(panel,selections_by_n[20],20,COSTS["base"],"minus100")
    ...
    summary = pd.DataFrame([metrics(combined[c].dropna(),baseline_date,c) for c in combined.columns])
    summary.to_csv(OUT/"summary_verified_period.csv",index=False,encoding="utf-8-sig")

    robust_df = pd.concat(robustness_nav,axis=1).sort_index()
    robust_metrics = pd.DataFrame([metrics(robust_df[c].dropna(),baseline_date,c) for c in robust_df.columns])
    robust_metrics.to_csv(OUT/"robustness_metrics.csv",index=False,encoding="utf-8-sig")
    ...
    meta = {
        ...
        "risk_free_rate":0.0,
        "metrics":"CAGR/volatility/Sharpe from monthly NAV; MDD/max recovery from daily NAV, matching attached v2-14.2 contract.",
