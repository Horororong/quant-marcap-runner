...
표준 퀀트 백테스트 템플릿 v2-12 / CURRENT (2026-09 업데이트)
...
6) 채팅 출력은 "실제로 보이는 인터랙티브 차트"를 최우선한다.
   - 네이티브 기간 선택 UI가 지원되면 기간 선택형으로 렌더링한다.
   - 지원되지 않거나 렌더링 안정성이 확인되지 않으면 단일 차트 인라인 fallback을 사용한다.
...
def combine_period_payloads(*payloads: Dict[str, Any]) -> Dict[str, Any]:
    ...
    - 기본 선택: 2001~현재
    - 전략/벤치마크 시리즈는 체크박스/토글로 켜고 끌 수 있게 한다.
    - 책 검증기간은 성과표에는 포함하지만 차트 기간 버튼에서는 제외한다.
    """
    if not payloads:
...
8. 채팅 기본 그래프는 정확히 3개다.
   - 누적자산
   - Log2 누적자산
   - Drawdown
9. 위 3개 그래프는 모두 채팅 내 인터랙티브 그래프로 렌더링한다.
10. 각 그래프 상단에 동일한 기간 버튼을 제공한다.
   - 2001~현재
   - 2021~현재
   - 최장~현재
   - 기본 선택은 2001~현재
11. 각 그래프에서 전략과 벤치마크 시리즈를 체크박스/토글로 켜고 끌 수 있게 한다.
12. 책 검증기간은 성과표와 책 수치 비교에는 포함하지만 기본 그래프의 기간 버튼에서는 제외한다.
13. 기본 출력 형태는 '성과표 + 기간 버튼이 있는 인터랙티브 그래프 3개'다.
14. matplotlib 정적그래프를 기본 수단으로 사용하지 않는다. 한글 깨짐 위험 때문에 채팅 내 인터랙티브 렌더링을 우선한다.
15. HTML/PNG/CSV 다운로드는 보조자료이며 채팅 인라인 그래프를 대체하지 않는다.
16. 차트가 화면에 실제로 나타나지 않았으면 '그래프를 제공했다'고 간주하거나 표현하지 않는다.
17. Log2 차트는 log2(NAV)를 직접 그려 1배, 2배, 4배, 8배...가 동일 간격이어야 한다.
18. 그래프 렌더링 전에 기간, 관측치, 누락치, 열 정렬을 검증한다.
19. 한글 제목, 범례, 축 라벨을 기본으로 사용한다.
20. 새 대화에서도 사용자가 단순히 '백테스트해줘'라고 요청하면 별도 지시 없이 이 CURRENT 템플릿의 계산 및 출력 규칙을 적용한다.
"""
...
def _self_test() -> None:
    ...
    # v2-12/CURRENT interactive-dashboard regression test.
    ...
    assert combined["chat_render_policy"]["inline_chart_count"] == 3
    assert combined["chat_render_policy"]["same_period_buttons_on_each_chart"] is True
    assert combined["chat_render_policy"]["series_toggles_required"] is True
