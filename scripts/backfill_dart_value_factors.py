...
def build_tasks(mapping: pd.DataFrame, now_kst: datetime) -> pd.DataFrame:
    ...
    for _, r in mapped.iterrows():
        ...
        for year in range(start_year, end_year + 1):
            # Needed for October rebalancing and TTM construction.
            if year < current_year or today >= pd.Timestamp(f'{year}-08-15'):
                rows.append({
                    'stock_code': r['stock_code'],
                    'corp_code': r['corp_code'],
                    'year': year,
                    'period': 'H1',
                    'reprt_code': REPORT_CODES['H1'],
                })
            # Needed for April rebalancing.
            if year < current_year and today >= pd.Timestamp(f'{year + 1}-04-01'):
                rows.append({
                    'stock_code': r['stock_code'],
                    'corp_code': r['corp_code'],
                    'year': year,
                    'period': 'FY',
                    'reprt_code': REPORT_CODES['FY'],
                })
    ...
    return (
        ...
        .sort_values(['year', 'period', 'stock_code'])
...
def save_candidates(new_rows: list[dict]) -> pd.DataFrame:
    ...
    if CANDIDATE_FILE.exists():
        old = pd.read_csv(CANDIDATE_FILE, dtype={'stock_code': str, 'corp_code': str}, compression='gzip')
        out = pd.concat([old, new], ignore_index=True)
    else:
        out = new
    if len(out):
...
def main() -> None:
    ...
    print(
        f'DART value backfill: mapped={mapped_count}/{len(mapping)} '
