...
RISK_MONTHS = {11, 12, 1, 2, 3, 4}  # 11월 초 진입 -> 5월 초 청산
...
def backtest(asset_ret: pd.DataFrame, profile_weights: pd.Series, cost_rate: float):
    ...
    sleeves = pd.Series(0.0, index=ASSETS)
    ...
    # Start in the regime appropriate for the first month. The sample starts Jan-2000,
    # so the initial allocation is the original K-All Weather profile.
    current_target = target_for_month(profile_weights, dates[0].month)
    sleeves = INITIAL_KRW * current_target

    for i, dt in enumerate(dates):
        # Rebalance only at the beginning of May and November.
        # Also rebalance at the first sample month to initialize the portfolio.
        if i == 0:
            traded = 0.0
            cost = 0.0
        elif dt.month in (5, 11):
            ...
            after_cost_nav = nav_before - cost
            sleeves = after_cost_nav * target
...
def metrics(nav: pd.Series, ret: pd.Series, rf: pd.Series, turnover: pd.Series, total_cost: float):
    ...
    downside = ret[ret < 0]
    sortino = float((ret.mean() - rf.mean()) / downside.std(ddof=1) * math.sqrt(12.0)) if len(downside) > 1 and downside.std(ddof=1) > 0 else np.nan
...
def baseline_metrics(baseline_nav: pd.Series, rf: pd.Series):
    ...
    # First month return is retained via initial capital because the existing baseline NAV
    # already reflects the first month's return.
    first_ret = baseline_nav.iloc[0] / INITIAL_KRW - 1.0
    ret.iloc[0] = first_ret
    dummy_turnover = pd.Series(0.0, index=baseline_nav.index)
    return metrics(baseline_nav, ret, rf, dummy_turnover, 0.0), ret
...
def main():
    ...
    # Existing baseline metrics for direct comparison.
    baseline_rets = {}
    for p in PROFILES:
        m, r = baseline_metrics(baseline_nav[p], rf)
    ...
    for cost_label, cost_rate in COST_SCENARIOS.items():
        for p in PROFILES:
            ...
            m = metrics(nav, ret, rf, turnover, costs.sum())
            m.update({
    ...
    summary.to_csv(OUT / "summary_compare.csv", index=False, encoding="utf-8-sig")
    ...
    # Three standard charts. Solid = seasonal, dashed = existing annual-rebalanced K-All Weather.
